        ...
        for epoch in tqdm(range(args.start_epochs, args.start_epochs + args.epochs)):
            ...
            for batch_idx, (x, y) in enumerate(train_loader):
                ...
                for i in range(len(triggered_labels)):
                    ...
                    # print(f"weights.shape: {weights.shape}, target.shape: {target.shape}")   #  8   4
                    # print( len(triggered_labels))   #  1
                    wm_loss = args.alpha1 * wm_loss_fn(weights.squeeze(dim=1), target.float().to(args.device)) / len(triggered_labels)
                    wm_loss.backward()
                optim.step()
...
def test(args):
    ...
    acc_val, _ = test_img(model, test_dataset, args)
    # acc_val, _ = test_img(model, train_dataset, args)
    logger.info("Primitive Accuracy: {:.3f}".format(acc_val))
    # print("Primitive Accuracy: {:.3f}".format(acc_val))
    ...
    logger.info("Average WSR:{}".format(np.average(wsrs)))
    logger.info("Chi2 Test: Average Corr:{}, Average P-value:{}".format(np.average(chi2_corrs), np.average(chi2_p_values)))
    print("Average WSR:{}".format(np.average(wsrs)))
    print("Chi2 Test: Average Corr:{}, Average P-value:{}".format(np.average(chi2_corrs), np.average(chi2_p_values)))
